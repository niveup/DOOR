...
if (isProduction && configuredOrigins.length === 0) throw new Error("ALLOWED_ORIGINS is required in production.");
const allowedOrigins: string[] = configuredOrigins.length ? configuredOrigins : ["http://localhost:3000"];
const prisma = new PrismaClient();
const app = express();
app.use(cors({ origin(origin, callback) { if (!origin || allowedOrigins.includes(String(origin))) return callback(null, true); return callback(new Error("Not allowed by CORS")); }, methods: ["GET", "POST", "PUT", "PATCH", "DELETE", "OPTIONS"], allowedHeaders: ["Content-Type", "x-passcode", "x-cron-secret"] }));
...
function cleanOldSessions() {
  ...
  for (const [key, val] of sessionStore.entries()) {
    if (now - val.lastUpdated > maxAge) sessionStore.delete(key);
  }
}

function recordSessionAttempt(sessionId: string, attempt: InMemAttempt) {
  cleanOldSessions();
  let session = sessionStore.get(sessionId);
  if (!session) {
    session = { attempts: new Map(), lastUpdated: Date.now() };
    sessionStore.set(sessionId, session);
  }
  session.attempts.set(attempt.questionIndex, attempt);
  session.lastUpdated = Date.now();
}

function getSessionSummary(sessionId: string, sessionLength: number) {
  const session = sessionStore.get(sessionId);
  if (!session) return null;
  const attempts = Array.from(session.attempts.values());
  if (attempts.length < sessionLength) return null;
  const scored = attempts.filter((item) => !item.skipped);
  return {
    completed: true,
...
app.post("/api/interview/evaluate", async (req, res) => {
  ...
  let id: ReturnType<typeof interviewIdentity>;
  try {
    id = interviewIdentity(body);
  } catch (error) {
    return res.status(400).json({ error: error instanceof Error ? error.message : "Invalid session." });
  }
  ...
  if (!company || !labels || question.length < 5 || question.length > 1200 || answer.length < 20 || answer.length > 8000) {
    return res.status(400).json({ error: "Interview input is invalid." });
  }
  const systemPrompt = [
  ...
  try {
    ...
    return res.json({
      ...feedback,
      attemptId,
      sessionSummary: getSessionSummary(id.sessionId, id.sessionLength),
    });
  } catch (error) {
