...
export default function StudyScreen() {
  ...
  const warnings = tracker.data?.subjects.filter((subject) => subject.isNeglected || subject.hasAvoidanceWarning) || [];
      ...
      {tracker.data ? (
        <>
          ...
          <Card style={styles.metrics}>
            ...
            <Metric label="At risk" value={warnings.length} accent={warnings.length ? colors.rose : colors.amber} />
          </Card>
          {warnings.length ? (
            <Card style={styles.warning}>
              <View style={styles.warningHeader}>
                <Ionicons name="warning-outline" size={19} color={colors.amber} />
                <Text style={styles.warningTitle}>Avoidance radar</Text>
              </View>
              <Text style={styles.warningText}>
                {warnings.slice(0, 3).map((subject) => subject.subjectName).join(", ")}{" "}
                {warnings.length > 3 ? `and ${warnings.length - 3} more` : ""} need an honest next session.
              </Text>
            </Card>
          ) : null}
          <SectionTitle title="Subject readiness" />
          <View style={styles.subjectList}>
            {tracker.data.subjects.map((subject) => (
              <SubjectRow key={subject.subjectId} subject={subject} />
            ))}
          </View>
          <SectionTitle
            title="Recent study logs"
            trailing={<ActionButton label="Log session" compact onPress={() => open("log")} />}
          />
          {tracker.data.logs.length ? (
            <View style={styles.logList}>
              {tracker.data.logs.slice(0, 5).map((log) => (
...
function SubjectRow({ subject }: { subject: TrackerSubject }) {
  const rating = subject.latestRating || 0;
  const tone =
    subject.hasAvoidanceWarning || subject.isNeglected
      ? colors.rose
      : rating >= 4
      ? colors.emerald
      : rating >= 3
      ...
      <View style={styles.subjectTop}>
        <View style={styles.subjectCopy}>
          ...
          <Text style={styles.subjectMeta}>
            {subject.cumulativeHours.toFixed(1)}h total · {subject.cumulativeQuestions} questions
          </Text>
        </View>
        <View style={[styles.rating, { borderColor: tone }]}>
          <Text style={[styles.ratingText, { color: tone }]}>{rating || "—"}/5</Text>
        </View>
      </View>
      <ProgressBar value={rating * 20} tone={tone} />
      {subject.isNeglected || subject.hasAvoidanceWarning ? (
        <Text style={[styles.flag, { color: tone }]}>
          {subject.hasAvoidanceWarning ? "Repeated avoidance pattern" : "No recent progress logged"}
        </Text>
      ) : null}
    </Card>
  );
...
const styles = StyleSheet.create({
  ...
  subjectTitle: { color: colors.text, fontSize: 14, fontWeight: "800" },
  subjectMeta: { color: colors.textFaint, fontSize: 11 },
  rating: { borderWidth: 1, borderRadius: 10, paddingHorizontal: 8, paddingVertical: 5 },
