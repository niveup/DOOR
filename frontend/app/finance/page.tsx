...
function BudgetModal({ budgetPlan, onClose, onSave }: { budgetPlan: BudgetPlan; onClose: () => void; onSave: (plan: BudgetPlan) => void }) {
  ...
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={onClose} className="fixed inset-0 z-50 flex items-end bg-black/35 p-0 backdrop-blur-[2px] sm:items-center sm:justify-center sm:p-4">
    <motion.form initial={{ opacity: 0, y: 18, scale: 0.985 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 18, scale: 0.985 }} transition={{ duration: 0.22, ease: [0.16, 1, 0.3, 1] }} onMouseDown={(event) => event.stopPropagation()} onSubmit={handleSave} role="dialog" aria-modal="true" aria-labelledby="budget-title" className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-2xl border border-[var(--border)] bg-[var(--bg-card)] p-5 shadow-2xl sm:rounded-2xl sm:p-6">
...
function BillsModal({ bills, onClose, onMarkPaid, onDelete, onAdd }: { bills: Bill[]; onClose: () => void; onMarkPaid: (bill: Bill) => void; onDelete: (id: string) => void; onAdd: (bill: Omit<Bill, "id" | "paid">) => void }) {
  ...
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={onClose} className="fixed inset-0 z-50 flex items-end bg-black/35 p-0 backdrop-blur-[2px] sm:items-center sm:justify-center sm:p-4">
    <motion.div initial={{ opacity: 0, y: 18, scale: 0.985 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 18, scale: 0.985 }} transition={{ duration: 0.22, ease: [0.16, 1, 0.3, 1] }} onMouseDown={(event) => event.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="bills-title" className="max-h-[92vh] w-full max-w-lg overflow-y-auto rounded-t-2xl border border-[var(--border)] bg-[var(--bg-card)] p-5 shadow-2xl sm:rounded-2xl sm:p-6">
...
function ExpenseModal({ form, onChange, onClose, onSubmit }: { form: { title: string; category: Category; amount: string; date: string; payment: Expense["payment"] }; onChange: (form: { title: string; category: Category; amount: string; date: string; payment: Expense["payment"] }) => void; onClose: () => void; onSubmit: (event: FormEvent<HTMLFormElement>) => void }) {
  return <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onMouseDown={onClose} className="fixed inset-0 z-50 flex items-end bg-black/35 p-0 backdrop-blur-[2px] sm:items-center sm:justify-center sm:p-4">
    <motion.form initial={{ opacity: 0, y: 18, scale: 0.985 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 18, scale: 0.985 }} transition={{ duration: 0.22, ease: [0.16, 1, 0.3, 1] }} onMouseDown={(event) => event.stopPropagation()} onSubmit={onSubmit} role="dialog" aria-modal="true" aria-labelledby="expense-title" className="w-full max-w-md rounded-t-2xl border border-[var(--border)] bg-[var(--bg-card)] p-5 shadow-2xl sm:rounded-2xl sm:p-6">
