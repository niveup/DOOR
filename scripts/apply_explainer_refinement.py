...
# Mechanical-only suggestions keep the product anchored to GATE Mechanical.
...
# Add semantic hooks without touching API/state behavior.
replacements = [
...
# Give the content stream a semantic wrapper hook using the existing flow comment anchor.
if "className=\"explainer-output-flow\"" not in text:
    anchor = '{thread.length > 0 ? ('
    if anchor in text:
        text = text.replace(anchor, '{thread.length > 0 ? (\n <div className="explainer-output-flow">', 1)
        # Close at the final branch before the outer conditional close.
        marker = '\n ) : null}'
...
# Distinct hooks for existing interactive renderer blocks.
for old, new in [
...
.explainer-output-flow {
  max-width: 78rem;
  margin-inline: auto;
}
.explainer-output-flow > * {
  animation: explainer-enter 240ms var(--ease-out) both;
}
.explainer-output-flow > *:nth-child(2) { animation-delay: 25ms; }
.explainer-output-flow > *:nth-child(3) { animation-delay: 50ms; }
.explainer-output-flow > *:nth-child(4) { animation-delay: 75ms; }
...
@keyframes explainer-enter { from { opacity: 0; transform: translateY(4px); } to { opacity: 1; transform: translateY(0); } }

@media (max-width: 900px) {
  .explainer-output-flow { width: 100%; }
  .explainer-session-bar { padding-inline: 1rem; }
  .explainer-formula-block { overflow-x: auto; }
}
@media (prefers-reduced-motion: reduce) {
  .explainer-output-flow > *, .explainer-session-bar, .explainer-section-toggle, .explainer-followup, .explainer-formula-block, .explainer-quiz button, .explainer-flashcard, .explainer-interview-cards button, .explainer-diagram img { animation: none; transition: none; }
}
'''
if '.explainer-output-flow' not in text:
    text += css_add
